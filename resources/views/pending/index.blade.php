                        ...
                        <table class="table table-hover table-bordered table-striped">
                            ...
                            @foreach($data as $row)
                                <?php
                                    if($row->flow_type=='single'){
                                        ...
                                        $duration = \App\Http\Controllers\ParamController::timeDiff($row->prepared_date,$now);
                                    }
                            ...
                            <tr>
                                ...
                                <td class="text-bold text-warning normal">
                                    ...
                                    @else
                                        <?php
                                            $u = \App\User::find($row->prepared_by);
                                            $s = \App\Section::find($u->section);
                                        ?>
                                        {{ $u->lname }}, {{ $u->fname }}
                                            <br />
                                            <small class="text-muted small-sub">
                                                @if(strlen($s->description)>30)
                                                    {{ $s->code }}
                                                @else
                                                    {{ $s->description }}
                                                @endif
                                ...
                                <td class="text-center" style="white-space: nowrap;">
                                    <div class="btn-group">
                                        <button data-toggle="modal" data-target="#trackDocument"  data-route_no="{{ $row->route_no }}" type="button" class="btn-track btn btn-sm btn-success"><i class="fa fa-line-chart"></i> Track</button>
                                        <button data-toggle="modal" data-target="#endDocument"  data-tracking_id="{{ $row->tracking_id }}" type="button" class="btn-end btn btn-sm btn-danger"><i class="fa fa-recycle"></i> End</button>
                                    </div>
...
@section('js')
    <script>
        ...
        $('.btn-end').on('click',function(){
            ...
            var url = "{{ url('track/end') }}/"+tracking_id;
            var loading = "{{ url('loading') }}";
            $('.cycle_content').load(loading);
            setTimeout(function(){
                $('.cycle_content').load(url);
            },1000);
        });
