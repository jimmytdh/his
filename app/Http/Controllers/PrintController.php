...
class PrintController extends Controller
{
    ...
    public function printDietaryReportChart()
    {
        $diet_list = DietaryController::getDietList();
        $data = array();
        $pdf = PDF::loadView('modules.dietary.print.chart',[
            'data' => $data,
            'diet_list' => $diet_list
        ])
        ->setPaper('a4','portrait');

        return $pdf->stream('Diet Census.pdf');
    }

    public function printDietaryReportRoom()
    {
        $room = Room::select(
...
//        return view('modules.dietary.print.room',[
//            'room' => $room
//        ]);
        $pdf = PDF::loadView('modules.dietary.print.room',[
            'room' => $room
        ])
            ->setPaper('a4','landscape');

        return $pdf->stream('Diet Per Room Report.pdf');
    }
}
